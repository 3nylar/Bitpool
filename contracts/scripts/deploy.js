...
// frontend/lib/contracts/deployments/<network>.json so the frontend can pick
// them up automatically.
async function main() {
  const [deployer] = await ethers.getSigners();
  console.log(
    `Deploying to network "${network.name}" from ${deployer.address}`,
  );
  ...
  await (
    await tokenB.ownerMint(deployer.address, (seedAmount / 2000n) * 1000n)
  ).wait(); // ~1 sETH = 2000 sUSD starting price
  await (
  ...
  console.log(
    "Seeded initial liquidity: 50,000 sUSD / 25 sETH (starting price 1 sETH = 2,000 sUSD)",
  );
